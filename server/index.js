...
function processRound(room) {
	...
	if (room.results.length >= 5) {
		const playerWins = room.results.filter(
			(r) => r.winner === "player"
		).length;
		const opponentWins = room.results.filter(
			(r) => r.winner === "opponent"
		).length;
...
function calculateRoundResult(playerAction, opponentAction) {
	...
	const getElementOutcome = (playerElement, opponentElement) => {
		...
		return winConditions[playerElement] === opponentElement
			? "win"
			: "lose";
	};
	...
	const outcome = getElementOutcome(
		playerAction.element,
		opponentAction.element
	);

	// Check if defense is used
	const isVoid =
		(playerAction.useDefense && outcome === "lose") ||
		(opponentAction.useDefense && outcome === "win");
	...
	if (!isVoid) {
		if (outcome === "win") {
			...
			if (playerAction.level > opponentAction.level) {
				winner = "player";
				damage =
					getLevelDamage(playerAction.level) -
					getLevelDamage(opponentAction.level);
	...
	for (const name of Object.keys(interfaces)) {
		for (const net of interfaces[name]) {
			if (net.family === "IPv4" && !net.internal) {
				console.log(
					`Accessible on your network at: http://${net.address}:${PORT}`
				);
			}
		}
	}
