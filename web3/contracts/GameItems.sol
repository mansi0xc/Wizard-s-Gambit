...
contract GameItems is ERC1155, AccessControl, ERC1155Burnable {
    ...
    uint256 public constant FIRE_CARD = 1001;
    uint256 public constant ICE_CARD = 1002;
    uint256 public constant WIND_CARD = 1003;

    // 🪄 Wands (Non-fungible)
    uint256[] public fireWands = [101, 102, 103];
    uint256[] public iceWands = [201, 202, 203];
    uint256[] public windWands = [301, 302, 303];
    ...
    function mintBasicSpell(uint256 spellId, uint256 elementCard) external {
        require(isValidBasicSpell(spellId), "Invalid spell ID");

        _burn(msg.sender, elementCard, 50); // Burn 50 element cards
        _mint(msg.sender, spellId, 1, ""); // Mint the Level 1 spell
    }


    // 🔥 Upgrade Spell (Requires Burning Cards & Having Wand)
    function upgradeSpell(uint256 oldSpell, uint256 newSpell, uint256 elementCard, uint256 wandId) external {
    ...
    function mintProfessor(uint256 shardId, uint256 professorId) external {
        ...
        require(userBalance >= 3, "Need at least 3 shards to mint professor");
